...
} from "@session.js/types/snode-retrieve";
...
export async function poll(
	this: BunNetwork,
	{ swarm, namespaces }: RequestPollBody,
): Promise<ResponsePoll> {
...
export async function pollSnode(
	this: BunNetwork,
	{
		swarm,
		namespaces,
	...
	return results.map(({ messages, namespace }) => ({
		namespace,
		messages: messages.messages as RetrieveMessageItem[],
	}));
}
...
export async function retrieveNextMessages(
	...
	try {
		...
		throw new SessionFetchError({
			code: SessionFetchErrorCode.InvalidResponse,
			message: "Couldn't parse json of " + targetNode.public_ip + ":" + targetNode.storage_port,
		});
	}
}
