...
export async function onRequest(
	...
	body: RequestStoreBody,
): Promise<ResponseStore>;
export async function onRequest(
	this: BunNetwork,
	type: RequestType.GetSnodes,
	body: object,
): Promise<ResponseGetSnodes>;
export async function onRequest(
	this: BunNetwork,
	type: RequestType.GetSwarms,
	body: RequestGetSwarmsBody,
): Promise<ResponseGetSwarms>;
export async function onRequest(
	this: BunNetwork,
...
export async function onRequest(
	...
	body: RequestDownloadAttachment,
): Promise<Uint8Array>;
export async function onRequest(
	this: BunNetwork,
...
export class BunNetwork implements Network {
	...
	onRequest(this: BunNetwork, type: RequestType, body: object): Promise<object> {
		const handler = onRequest as (
			this: BunNetwork,
			type: RequestType,
			body: object,
		) => Promise<object>;

		return handler.call(this, type, body);
	}
}
